...
ARCHETYPE_CATEGORY = {
    ...
    "dialogue":          4,   # правовое QA §2.4 и рассуждение §2.5
    "code_switching":    7,
    "cultural_refusal":  8,
    "benchmark":         6,
}
...
def manifest():
    ...
    if request.method == "POST":
        ...
        if not batch_id:
            flash(tt("manifest.err_batch_id"), "error")
            return redirect(url_for("manifest"))
        ...
        # Собираем аннотаторов и рецензентов из единиц
        annotators = sorted({user["full_name"]})   # текущий пользователь всегда включён
        reviewers  = set()
        for r in rows:
            if r["reviewer"]:
                reviewers.add(r["reviewer"].strip())
        # Дополнительные рецензенты введённые вручную
        ...
        # Категория: берём по большинству единиц
        ...
        # Имя DOCX-файла совпадает с batch_id
        docx_filename = f"{batch_id}.docx"
        ...
        bio = BytesIO(json.dumps(manifest_data, ensure_ascii=False, indent=2).encode("utf-8"))
        bio.seek(0)
        return send_file(
            bio,
            as_attachment=True,
    ...
    # Предлагаем batch_id по умолчанию
    ym = dt.datetime.now().strftime("%Y_%m")
    default_batch_id = f"batch_{ym}_<категория>_01"
    return render_template(
        "manifest.html",
        units=units,
