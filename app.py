...
def _load_secret_key():
    """Читает SECRET_KEY из файла secret.key рядом с app.py.
    При первом запуске генерирует и сохраняет ключ — он не меняется при перезапуске."""
...
app = Flask(__name__)
app.config["SECRET_KEY"] = os.environ.get("SECRET_KEY") or _load_secret_key()
...
ARCHETYPES = [
    ("business_scenario", "§2.1 — Бизнес-сценарий с инструментами"),
    ("preference",        "§2.2 — Пара предпочтений (DPO)"),
    ("cultural_reasoning","§2.3 — Культурное рассуждение"),
    ("legal_qa",          "§2.4 — Правовое и нормативное QA"),
    ("reasoning_agent",   "§2.5 — Многотуровый агент с рассуждением"),
    ("benchmark",         "§2.6 — Бенчмарк (удерживаемый)"),
    ("code_switching",    "§2.7 — Переключение кода и регистра"),
    ("cultural_refusal",  "§2.8 — Культурный отказ и перенаправление"),
]
...
def login():
    ...
    if request.method == "POST":
        ...
        if row and check_password_hash(row["password_hash"], password):
            session.clear()
            session.permanent = True          # живёт 30 дней (PERMANENT_SESSION_LIFETIME)
...
def reset_password():
    """Смена пароля без email — пользователь вводит логин и новый пароль."""
    if request.method == "POST":
        ...
        if not username:
            errors.append(tt("reset.err_username_required"))
        if len(password) < 8:
            errors.append(tt("reg.err_password_short"))
        if password != password2:
            errors.append(tt("reg.err_password_mismatch"))
        ...
        if not errors:
            ...
            if not user:
                errors.append(tt("reset.err_not_found"))
        ...
        db.commit()
        flash(tt("reset.success"), "success")
        return redirect(url_for("login"))

    return render_template("reset_password.html", username="")
...
# Маппинг архетип → номер категории по инструкции v3
ARCHETYPE_CATEGORY = {
    "business_scenario":  1,
    "preference":         2,
    "cultural_reasoning": 3,
    "legal_qa":           4,
    "reasoning_agent":    5,
    "benchmark":          6,
    "code_switching":     7,
...
@login_required
def manifest():
    """Генерирует manifest.json для батча по выбранным единицам."""
    db   = get_db()
    user = current_user()
    ...
    if request.method == "POST":
        ...
        defect_rate_str     = request.form.get("defect_rate", "0").strip()
        synthetic_pct_str   = request.form.get("synthetic_pct", "0").strip()
        extra_files_str     = request.form.get("extra_files", "").strip()
        extra_reviewers_str = request.form.get("extra_reviewers", "").strip()

        errors = []
        if not batch_id:
            errors.append(tt("manifest.err_batch_id"))

        try:
            defect_rate = round(float(defect_rate_str), 4)
            if not (0.0 <= defect_rate <= 1.0):
                errors.append(tt("manifest.err_defect_range"))
        except ValueError:
            defect_rate = 0.0

        try:
            synthetic_pct = round(float(synthetic_pct_str), 2)
            if not (0.0 <= synthetic_pct <= 100.0):
                errors.append(tt("manifest.err_synth_range"))
        except ValueError:
            synthetic_pct = 0.0
        ...
        # Единицы с именем аннотатора через JOIN
        placeholders = ",".join("?" * len(ids))
        rows = db.execute(
            f"""SELECT u.*, us.full_name AS annotator_name
                FROM units u
        ...
        # Аннотаторы — из реальных владельцев единиц (JOIN с users)
        from collections import Counter
        annotators  = sorted({r["annotator_name"] for r in rows})
        ann_counts  = Counter(r["annotator_name"] for r in rows)

        # Предупреждения по инструкции §4.2
        if len(annotators) < 3:
            flash(tt("manifest.warn_annotators", len(annotators)), "warning")
        for name, cnt in ann_counts.items():
            if cnt / total > 0.40:
                flash(tt("manifest.warn_ann_share", name, round(cnt / total * 100)), "warning")
        if synthetic_pct > 10.0:
            flash(tt("manifest.warn_synth_limit"), "warning")

        # Рецензенты: из поля reviewer каждой единицы + ручной ввод
        reviewers = set()
        for r in rows:
            if r["reviewer"]:
                reviewers.add(r["reviewer"].strip())
        for name in extra_reviewers_str.split(","):
            name = name.strip()
            if name:
                reviewers.add(name)
        reviewers = sorted(reviewers)

        # Категория: один батч = одна категория (§3.3)
        cat_counts = Counter(
            ARCHETYPE_CATEGORY.get(dict(r)["archetype"], 0) for r in rows
        )
        if len(cat_counts) > 1:
            flash(tt("manifest.warn_mixed_category"), "warning")
        category = cat_counts.most_common(1)[0][0]

        # Файлы: основной DOCX + дополнительные из поля
        files = [f"{batch_id}.docx"]
        for f in extra_files_str.split(","):
            f = f.strip()
        ...
        manifest_data = {
            ...
            "declared_internal_qa_defect_rate":  defect_rate,
            "synthetic_percentage_overall":      synthetic_pct,
            "schema_version":                    "v3",
        }
    ...
    # GET — все единицы всех пользователей (руководитель батча собирает общий манифест)
    units = db.execute(
        """SELECT u.id, u.unit_code, u.archetype, u.domain, u.created_at,
                  us.full_name AS annotator_name
           FROM units u
           JOIN users us ON us.id = u.user_id
           ORDER BY u.id DESC""",
    ).fetchall()
    ym = dt.datetime.now().strftime("%Y_%m")
    default_batch_id = f"batch_{ym}_business_01"
    return render_template(
        "manifest.html",
        units=units,
